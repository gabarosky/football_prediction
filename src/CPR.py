...
class CPRModel:
    """
    A class to store CPR ratings and parameters, 
    ...
    def predict(self, home_team, away_team, avg_goals = 1.3, neutral=True):
        ...
        loc = 0.0 if neutral else self.params['loc']
        diff = self.ratings[h_id] - self.ratings[a_id] + loc
        lambda_h = avg_goals * (10**(diff / self.params['div']))
        lambda_a = avg_goals * (10**(-diff / self.params['div']))
        margin = self.params['draw']
        p_away = 1 / (1 + 10 ** ((diff + margin) / self.params['div']))
        p_home = 1 / (1 + 10 ** (-(diff - margin) / self.params['div']))
    ...
    def predict_probs(self, home_team, away_team, neutral=True):
        ...
        delta = self.ratings[h_id] + loc - self.ratings[a_id]
        delta = rat_a + loc -rat_b
        margin = self.params['draw']
        
        p_away = 1 / (1 + 10 ** ((delta + margin) / self.params['div']))
        p_home = 1 / (1 + 10 ** (-(delta - margin) / self.params['div']))
        p_draw = max(0.0, 1.0 - p_home - p_away)
    ...
    def simulate_match(self,home_team, away_team, rng , avg_goals=1.3):
        ...
        diff = elo_a - elo_b
        lambda_a = avg_goals * (1/(1+10**(-diff / self.params['div'])))
        lambda_b = avg_goals * (1/(1+10**(diff / self.params['div'])))
