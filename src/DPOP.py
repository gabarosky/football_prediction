...
class DPOPModel:
    """
    A class to store DPOP strength and parameters, 
    allowing for easy match predictions.
    """
    def __init__(self, alphas, betas, k1, k2, gamma, rho , team_to_id,xi):
        self.alphas = alphas
        self.betas = betas
        self.k1 = k1
        self.k2 = k2
    ...
    def get_rating(self, team_name):
        ...
        return  self.k1*self.alphas[team_id]+  self.k2*self.betas[team_id]
    ...
    def predict(self, home_team, away_team, avg_goals = 1.3, neutral=True):        
        ...
        lx = np.clip(np.exp(self.gamma) * (self.alphas[h_idx]**self.k1) / (self.betas[a_idx]**self.k2), 1e-3, 15.0)
        my = np.clip((self.alphas[a_idx]**self.k1) / (self.betas[h_idx]**self.k2), 1e-3, 15.0)
        
        
        """Calcula p_home, p_draw, p_away distribuyendo la corrección tau de forma correcta."""
        max_goals = 8
        ...
        for x in range(max_goals):
          for y in range(max_goals):
              ...
              p_x = (lx**x) * np.exp(-lx) / special.gamma(x+1)
              p_y = (my**y) * np.exp(-my) / special.gamma(y+1)
    
              # Aplicar factor tau
              tau = 1.0
              if x == 0 and y == 0:
                  tau = 1.0 - lx * my * self.rho
              elif x == 0 and y == 1:
                  tau = 1.0 + lx * self.rho
              elif x == 1 and y == 0:
                  tau = 1.0 + my * self.rho
      ...
      # Normalizar matriz para asegurar consistencia probabilística
    ...
    def simulate_match(self, home_team, away_team, rng,
      ...
      # Normalizar matriz para asegurar consistencia probabilística
        ...
        plain_results = p_matrix.flatten()
       ...
       # obtain the mode
        ...
        np.sum(np.tril(p_matrix, -1)), np.sum(np.diag(p_matrix)),
        
        e_b = np.ones(max_goals+1) @ p_matrix @ np.arange(max_goals+1)
        e_a = np.arange(max_goals+1) @p_matrix @ np.ones(max_goals+1)
        prob_local=np.sum(np.triu(p_matrix, -1))#resultado.prob_local,
        prob_empate=np.sum(np.diag(p_matrix))#resultado.prob_empate,
        prob_visita=np.sum(np.triu(p_matrix, 1))#resultado.prob_visita,
        ...
        val= {
            "prob_matrix"  : p_matrix, #resultado.matriz,
